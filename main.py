...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'ReaDiary'
session = {}
...
class UserModel:
    ...
    def insert(self, user_name, password_hash):
        ...
        cursor.execute('''INSERT INTO users 
                       ...
                       {"user_name": str(user_name), "password_hash":
                           hashlib.sha224(password_hash.encode('utf-8')).hexdigest()})
        cursor.close()
        self.connection.commit()
    ...
    def exists(self, user_name, password_hash):
        ...
        cursor.execute("SELECT * FROM users WHERE user_name = :user_name AND password_hash = :password_hash",
                       {"user_name": str(user_name), "password_hash": hashlib.sha224(password_hash.encode('utf-8')).hexdigest()})
        row = cursor.fetchone()
